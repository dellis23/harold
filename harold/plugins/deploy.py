...
class DeployMonitor(object):
    ...
    def acquire(self, irc, sender, channel):
        ...
        self._update_topic()
        self._notify_conch_transfer()

    def _notify_conch_transfer(self):
        if self.queue and self.queue[0] != self.current_conch:
            new_conch = self.queue[0]
            self.irc.bot.send_message(
                self.config.channel, "%s, you have the :conch:" % new_conch)
        else:
            new_conch = None
